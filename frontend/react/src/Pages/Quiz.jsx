...
const Quiz = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setLoading(false);
    }, 500); // Reduced time to make it load faster
  ...
  const handleChange = (id, value) => {
    ...
    // Reset submission state when user changes an answer
    if (submitted) {
      setSubmitted(false);
      setFeedback('');
    }
    ...
    questions.forEach(q => {
      if (q.correctAnswer) {
        ...
        if (q.type === 'radio' && answers[q.id] === q.correctAnswer) {
          ...
          // Check if arrays are equal (same items, regardless of order)
          const isCorrect = 
  ...
  const handleSubmit = () => {
    ...
    const unansweredQuestions = questions
      .filter(q => q.type !== 'paragraph') // Paragraph answers are optional
                ...
                {questions.map((q, qIndex) => (
                  <div key={q.id} className="p-5 rounded-lg border border-gray-200">
                    <div className="flex items-start gap-3 mb-3">
                      <span className="bg-blue-600 text-white font-bold rounded-full w-7 h-7 flex items-center justify-center flex-shrink-0">
                        {qIndex + 1}
                      </span>
